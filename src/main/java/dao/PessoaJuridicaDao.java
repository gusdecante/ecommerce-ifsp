...
public class PessoaJuridicaDao {
    ...
    public void registerPessoaJuridica(PessoaJuridica pj) throws SQLException {
        String query;
        //First - Insert User
        query = "INSERT INTO user (id_User, email, password, user_Tyoe, token) VALUES (?, ?, ?, ?, ?);";
        PreparedStatement st = con.prepareStatement(query);
        st.setInt(1, pj.getIdUser());
        st.setString(2, pj.getEmail());
        st.setString(3, pj.getPassword());
        st.setInt(4, pj.getTypeOfUser());
        st.setString(5, pj.getToken());
        st.execute();
